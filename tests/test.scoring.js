...
describe('scoring tests', () => {
    let /*Game*/game, /*Round*/round, /*Hand*/hand;
    ...
    describe('scoring special sets', () => {
        it('scoring minor special set, non mahjong', () => {
            hand.setSpecialSet(SpecialSets.minor);

            round.setHand(game.players[1], hand);

            scoreHand(round, game.players[1], game.roundLimit * 0.2);
        });

        it('scoring minor special set, mahjong', () => {
            hand.setSpecialSet(SpecialSets.minor);

            round.setHand(game.players[1], hand);
            round.setWinner(game.players[1], false, false);

            scoreHand(round, game.players[1], game.roundLimit * 0.5);
        });

        it('scoring major special set, non mahjong', () => {
            hand.setSpecialSet(SpecialSets.major);

            round.setHand(game.players[1], hand);

            scoreHand(round, game.players[1], game.roundLimit * 0.4);
        });

        it('scoring major special set, mahjong', () => {
            hand.setSpecialSet(SpecialSets.major);

            round.setHand(game.players[1], hand);
            round.setWinner(game.players[1], false, false);

            scoreHand(round, game.players[1], game.roundLimit);
        });

        it('scoring minor special set with hand content', () => {
            hand.setSpecialSet(SpecialSets.minor);
            hand.addSet(new Pung(false, Tiles.Circle4));
            hand.addSet(new FreeTiles([Tiles.BonusSummer, Tiles.BonusFlower1]));
            hand.addSet(new Kong(false, Tiles.DragonWhite));
